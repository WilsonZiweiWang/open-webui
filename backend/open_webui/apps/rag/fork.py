#
# Modified from: https://github.com/1Panel-dev/MaxKB/blob/main/apps/common/util/fork.py
#

# TODO: Modify the log message prefix to be more specific to the app
import copy
import logging
import re
import traceback
...
class ForkManage:
    ...
    def fork(self, level: int, exclude_link_url: Set[str], fork_handler,  link_list: list[str]):
        self.fork_child(ChildLink(self.base_url, None), self.selector_list, level, exclude_link_url, fork_handler,  link_list)

    @staticmethod
    def fork_child(child_link: ChildLink, selector_list: List[str], level: int, exclude_link_url: Set[str],
                   fork_handler, link_list: list[str]):
        if level < 0:
            return
        else:
        ...
        if not exclude_link_url.__contains__(child_url):
            ...
            fork_handler(child_link, response, link_list)
            for child_link in response.child_link_list:
                child_url = child_link.url[:-1] if child_link.url.endswith('/') else child_link.url
                if not exclude_link_url.__contains__(child_url):
                    ForkManage.fork_child(child_link, selector_list, level - 1, exclude_link_url, fork_handler, link_list)
